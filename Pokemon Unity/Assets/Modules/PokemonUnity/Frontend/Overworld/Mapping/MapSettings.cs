...
/// Purpose: Map scenery settings for Pokémon Unity frontend
...
public class MapSettings : MonoBehaviour
{
    ...
    public Sprite getBattleBackground(int currentTag = 0)
    {
        ...
        RaycastHit[] hitColliders =
            Physics.RaycastAll(Player.player.transform.position + new Vector3(0, 1.5f, 0), Vector3.down);
        Bridge bridge = null;
        ...
        if (hitColliders.Length > 0)
        {
            ...
            for (int i = 0; i < hitColliders.Length && bridge == null; i++)
            {
                ...
                if (hitColliders[i].collider.gameObject.GetComponent<Bridge>() != null)
                {
                    bridge = hitColliders[i].collider.gameObject.GetComponent<Bridge>();
